...
TimedOut = 'TimedOut'
timeout = 1.   # set timeout to 1 second to speed up process
mac = None
...
class Pinger:
# can be passed a hostname or ip address; if host_name is None, ip_address is used
    def __init__(self, host_name, ip_address, num):
        self.num = num
        self.last = 0
        self.sent = 0
        self.times = {}
        ...
        if host_name is None:
            self.sock = PingSocket(ip_address)
        else:
            self.sock = PingSocket(host_name)
        ...
        if host_name is None:
            ...
            self.addr = host_name
            name, aliases, ipaddr = socket.gethostbyaddr(host_name)
    ...
    def get_summary(self):
        ...
        return dmin, davg, dmax, sent, recv, loss, mac
